...
### v0.6.4 — 2026-05-31
- **服薬チェック「寝る前の薬」保存時のバリデーションバグ修正**: 服薬設定で「寝る前の薬（`before_sleep`）」を有効にしているユーザーにおいて、保存時にサーバー側でタイムポイントの不一致（400 Bad Request）が発生して「保存に失敗しました」と赤警告が表示され、記録が一切保存できなくなっていた致命的なバリデーション判定バグを修正。服薬データの場合のみ `before_sleep` を含む適切な時間帯を許容するように改善

### v0.6.3 — 2026-05-31
- **アトミック一括保存トランザクションの導入**: 記録の保存処理（`/api/save`）において、保存開始前に入力値の検証と型変換（パース）を完全に終わらせるように順序を改善。さらに、症状スコア、備考、WBGTの一連のDB保存を一括した単一のトランザクション内で実行するように統合。これにより、エラー時にスコアだけが中途半端に保存されて「保存に失敗しました」と表示されるデータ不整合（齟齬）を完全に解消

### v0.6.2 — 2026-05-30
- **新規登録症状の保存バリデーション緩和**: 非表示（非アクティブ化）にした症状のスコアデータがオートセーブ等のタイミングで送信された際、APIがエラーを返して以降すべての保存処理がブロックされる致命的なバグを修正。有効・無効を問わず登録済みの全症状を許容するようバリデーションを改善
- **設定画面での重複IDエラー通知表示**: すでに存在する症状名と同じIDで新規症状を追加しようとした際、画面上に上品な警告アラートを表示し、UXを向上
...
- **印刷レポート表示モードの切り替えUI**: 期間が長くなった際にページ数が莫大になるのを防ぐため、1日1行の簡潔な「一覧表示（クロス集計表）」を新設。従来の「詳細表示（全出力）」とスイッチで動的に切り替えられ、現在表示されているビューだけが自動的にA4・PDF印刷（印刷用メディアクエリ完全連動）される仕様を実装
